...
The formulation ideas behind this model emerged partly from conceptual discussions supported by ChatGPT \cite{chatgpt} (version~5.2), which helped explore alternative modeling perspectives and refinements.
...
The problem has different symmetries and we tried to reduce the search space as much as possible, but maintaining some flexibility. Some of the subsequent constraints seem redundant, but we experimentally found out they were improving performance by helping propagation, especially focusing on the cases in which $n$ was higher.
...
In the following table we reported the imbalance values of the best solution found by the solver within the time limit of 5 minutes.
Is important to notice that the values in the table are the values of the $max$ objective function implemented for CP, introduced in section \ref{CP obj function}.
Optimal results are marked in \textbf{bold}, \texttt{UNSAT} if the problem is proven infeasible and \texttt{N/A} if no result was found after the time limit.
% In this table, timing information of the solutions are not reported, but is important to know that the same solution found by the models with symmetry breaking constraints are obtained in less time with respect to those found by the standard model, e.g Chuffed-SB found the optimal solution for n=14 in 99 seconds, while Chuffed+SB found it in 68 seconds.
For non-optimal solutions, also the value of the original objective function is shown in brackets.
...
18 & \texttt{N/A}&\texttt{N/A}& 13 (84) &  3 (42)     \\
20 & \texttt{N/A}&\texttt{N/A}& 15 (156) &\texttt{N/A}\\
...
Among all models we developed, unless one specific case ($n$=18 with Chuffed solver), we had a good improvement for each of them when symmetry breaking (SB) constraints were applied.
...
\bibitem{chatgpt}
OpenAI. \emph{ChatGPT (GPT-5.2)}. 2026. Available at: \url{https://openai.com/}
