...
\subsection{Objective function}\label{obj_function exple}
...
a_t = W - h_t.
\]
The home/away imbalance of team $t$ is therefore:$\quad|h_t - a_t|.$
...
We don't need to explicitly impose that exactly two teams
...
\begin{itemize}
    ...
    \begin{align*}
        \forall p =1,\dots,P\qquad   Y_{1,p,(2p-1,\,2p)}=1\quad \text{and} \quad
        H_{1,p,(2p-1,\,2p)}=1
    \end{align*}
...
The interpretation of the reported results and the
meaning of the table entries follow the same convention described
in Section 2.4. For the decision column we just reported if we got a solution and the time it took to reach it.
...
6  & Yes (0s)  & \textbf{6} (0s)  \\
8  & Yes (0s)  & \textbf{8} (0s)   \\
10 & Yes (7s)  & \textbf{10} (12s) \\
12 & Yes (89s) & \textbf{12} (283s)  \\
14 & No       & \texttt{N/A}  \\
\hline
\end{tabular}
\caption{CBC results for the MIP model.}
